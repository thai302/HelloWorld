...
public class GSM7BitConverter {
    ...
    public static Byte[] stringToGsm7BitPackUssd(String msg) {
        Byte[] gsmBytes = stringToGsm7Bit(msg);
        Byte[] answer = packByteArrayUssd(gsmBytes);
        return answer;
    }
    ...
    public static String stringToGsm7BitPackHexUssd(String msg) {
        Byte[] bytes = stringToGsm7BitPackUssd(msg);
        return byteArrayToHexString(bytes);
    }
    ...
    public static Byte[] packByteArray(Byte[] gsmBytes) {
        ...
        if (gsmBytes.length == 1) {
            ...
            for (int i = 0; i < gsmBytes.length; i++) {
                ...
                Byte currentByte = gsmBytes[i];
                Byte nextByte = (byte) 0;
                if (i < (gsmBytes.length - 1)) {
                    nextByte = gsmBytes[i + 1];
                }

                int r_byte = currentByte >> r_shift;
                int l_byte = nextByte << l_shift;
    ...
    public static Byte[] packByteArrayUssd(Byte[] gsmBytes) {
        Byte[] result = packByteArray(gsmBytes);

        if ((gsmBytes.length % 8) == 7) {
            Byte lastByte = result[result.length - 1];

            //<CR> char
            int charCR = 0x0D;
            lastByte = (byte) (lastByte | (charCR << 1));
